...
export default function OfferingsClient({ initialData }: { initialData: Category[] }) {
  ...
  useRealtime(["offering_categories", "offering_sub_categories"], refreshOfferings);

  const activeCategory = categories[activeTab] || categories[0];
  if (!activeCategory) {
    return (
      <div style={{ display: "flex", justifyContent: "center", padding: "5rem", fontFamily: "'DM Sans', sans-serif" }}>
        <p style={{ color: "var(--text-mid)" }}>No offerings are announced currently. Check back soon!</p>
      </div>
    );
  }
